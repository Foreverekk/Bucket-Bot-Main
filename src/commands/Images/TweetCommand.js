...
module.exports = class TweetCommand extends BaseCommand {
  constructor() {
    super('tweet', 'images', ['twitter', 'twt']);
  }
  ...
  async run(client, message, args, AV, CMD, PREFIX, embederr) {
    ...
    try {
        ...
        let res = await fetch(encodeURI(`https://nekobot.xyz/api/imagegen?type=tweet&username=${member.user.username}&text=${text}`));
        let json = await res.json();
        let attachment = new Discord.MessageAttachment(json.message, `tweet-${message.member.user.username}.png`);
