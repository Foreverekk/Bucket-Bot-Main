...
module.exports = class EmojiDeleteEvent extends BaseEvent {
  ...
  async run(client, emoji) {
    ...
    if (db.get(`${emoji.guild.id}_logs`) === "on") {
      ...
      .setColor("WHITE")
      .setAuthor("Usunięto Emotkę", `https://cdn.discordapp.com/emojis/${emoji.id}.${emoji.animated ? "gif" : "png"}`, `https://cdn.discordapp.com/emojis/${emoji.id}.${emoji.animated ? "gif" : "png"}`)
